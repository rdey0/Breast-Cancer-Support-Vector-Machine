...
def optimize_rbf(x_train, x_test, y_train, y_test, max_degree, max_cost):
    ...
    for degree in range(1, max_degree + 1):
        for cost in range(1, max_cost +1):
            # create rbf model
            clf = svm.SVC(kernel='rbf', degree = degree, C=cost, gamma='scale')
...
"""
def get_graph(x1_label, x2_label, x_train, x_test, y_train, y_test, degree, cost):
...
best_degree, best_cost, max_accuracy, results = optimize_rbf(x_train, x_test, y_train, y_test,5,10)
get_graph(x1_label, x2_label, x_train, x_test, y_train, y_test, best_degree, best_cost)
